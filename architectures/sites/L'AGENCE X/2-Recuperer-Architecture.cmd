...
setlocal
REM ============================================================
REM  L'AGENCE X - recupere architecture + permissions.
...
if "%CODE%"=="0" (
    if exist "%~dp0resultats\architecture-latest.html" (
        echo Ouverture du resultat visualise...
        start "" "%~dp0resultats\architecture-latest.html"
        if exist "%~dp0Visualiseur-Architecture.html" (
            echo Ouverture de l'editeur de droits...
            start "" "%~dp0Visualiseur-Architecture.html"
        )
    ) else (
        echo [INFO] Ouvrez Visualiseur-Architecture.html puis choisissez resultats\architecture-latest.json.
    )
) else (
    echo [ERREUR] Recuperation non terminee. Voir resultats\recuperation-log.txt.
)
echo.
pause
exit /b %CODE%
